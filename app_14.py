...
def create_app8_ui():
  ...
  main_layout = html.Div(style={'background-color':''},children=[
  html.H1('Terrorism Analysis with Insights', id='Main_title',
          style={'background-color':'#cc0000','color':'#FFFFFF','font-size':'35px','textAlign':'center'}
          ),
  dcc.Tabs(id="Tabs", value="Map",
           style={'background-color': '#008080' ,'color':'#ffffff','font-size':'20px'},
           children=[
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map",
          ...
          dcc.Dropdown(
              id='month', 
                ...
                multi = True,style={
                             ...
                             'background-color': '#d9d9d9 ' ,'color':'#000000','font-size':'20px','border-radius':'40px'}
                  ),
          ...
          dcc.Dropdown(
                ...
                multi = True,
                style={'background-color': '#d9d9d9 ' ,'color':'#000000','font-size':'20px','border-radius':'40px'}
                  ),
      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart",
              style={'background-color': '#008080 ' ,'color':'#ffffff','font-size':'20px'}, children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",
            ...
            dcc.Dropdown(id="Chart_Dropdown", options = chart_dropdown_values, placeholder="Select option", value = "region_txt",
                         style={'background-color': '#d9d9d9 ' ,'color':'#000000','font-size':'20px'}), 
            html.Br(),
            html.Br(),
            html.Hr(),
            dcc.Input(id="search", placeholder="Search Filter",
                      style={'background-color':'#9999ff','color':'#00000'}),
            html.Hr(),
            html.Br(),
            dcc.RangeSlider(
                    id='year_slider1',
...
def update_app9_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value,chart_year, chart_value, search,
    ...
    if Tabs == "Map":
        ...
        if chart_value is not None and chart_df.shape[0]:
            if search is not None:
                chart_df = chart_df.groupby("iyear")[chart_value].value_counts().reset_index(name = "count")
                chart_df  = chart_df[chart_df[chart_value].str.contains(search, case=False)]
            else:
                chart_df = chart_df.groupby("iyear")[chart_value].value_counts().reset_index(name="count")
        ...
        if chart_df.shape[0]:
            ...
            chart_df = pd.DataFrame(columns = ['iyear', 'count', chart_value])
            
            chart_df.loc[0] = [0, 0,"No data"]
        chartFigure = px.area(chart_df, x="iyear", y ="count", color = chart_value)
        fig = chartFigure
    return dcc.Graph(figure = fig)
#filtering for dropdown data,like month and date relted or without month date not change
@app.callback(
  Output("date", "options"),
  [Input("month", "value")])
